...
autocmd User SneakLeave highlight clear SneakScope
    ...
    cmd(
        [[autocmd BufWinEnter,BufRead,BufNewFile * setlocal fo-=c fo-=r fo-=o fo+=t]])

    -- twpayne/chezmoi
    cmd(
        [[autocmd BufWritePost ~/.local/share/chezmoi/* :silent! !chezmoi apply --source-path %]])
    cmd(
        [[autocmd BufLeave ~/.config/cheatsheet.md :silent! !chezmoi add ~/.config/cheatsheet.md]])
end
...
local function sneak() cmd([[let g:sneak#use_ic_scs = 1]]) end
...
local function highlightedyank() g.highlightedyank_highlight_duration = 100 end
