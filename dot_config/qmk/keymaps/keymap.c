#include QMK_KEYBOARD_H

// US layout altgr-intl variant with french special characters
enum custom_keycodes {
    GRAVEA = SAFE_RANGE,
    GRAVEE,
    GRAVEU,
    CIRCUMFLEXA,
    CIRCUMFLEXO,
    CIRCUMFLEXE,
    CIRCUMFLEXU,
    CIRCUMFLEXI,
    TREMAE,
    TREMAU,
    TREMAI,
};

bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
    case GRAVEA:
        if (record->event.pressed) {
        SEND_STRING(SS_RALT("`")"a");
        }
        break;
    case GRAVEE:
        if (record->event.pressed) {
        SEND_STRING(SS_RALT("`")"e");
        }
        break;
    case GRAVEU:
        if (record->event.pressed) {
        SEND_STRING(SS_RALT("`")"u");
        }
        break;
    case CIRCUMFLEXA:
        if (record->event.pressed) {
        SEND_STRING(SS_RALT("6")"a");
        }
        break;
    case CIRCUMFLEXO:
        if (record->event.pressed) {
        SEND_STRING(SS_RALT("6")"o");
        }
        break;
    case CIRCUMFLEXE:
        if (record->event.pressed) {
        SEND_STRING(SS_RALT("6")"e");
        }
        break;
    case CIRCUMFLEXU:
        if (record->event.pressed) {
        SEND_STRING(SS_RALT("6")"u");
        }
        break;
    case CIRCUMFLEXI:
        if (record->event.pressed) {
        SEND_STRING(SS_RALT("6")"i");
        }
        break;
    case TREMAE:
        if (record->event.pressed) {
          SEND_STRING(SS_RALT(SS_RSFT("\""))"e");
        }
        break;
    case TREMAU:
        if (record->event.pressed) {
          SEND_STRING(SS_RALT(SS_RSFT("\""))"u");
        }
        break;
    case TREMAI:
        if (record->event.pressed) {
          SEND_STRING(SS_RALT(SS_RSFT("\""))"i");
        }
        break;
    }
    return true;
};

enum combos { 
  L1CTL,
  L1SFT,
  L1GUI,
  L1ALT,
  L2CTL,
  L2SFT,
  L2GUI,
  L2ALT,
  L1CS,
  L1GS,
  L2CS,
  L2GS,
  L3ACUTEE,
  L3GRAVEA,
  L3GRAVEE,
  L3GRAVEU,
  L3CIRCUMFLEXA,
  L3CIRCUMFLEXO,
  L3CIRCUMFLEXE,
  L3CIRCUMFLEXU,
  L3CIRCUMFLEXI,
  L3TREMAE,
  L3TREMAU,
  L3TREMAI,
  L3CEDILLAC,
};

const uint16_t PROGMEM l1ctl[] = { LT(1,KC_TAB), KC_U, COMBO_END};
const uint16_t PROGMEM l1sft[] = { LT(1,KC_TAB), KC_E, COMBO_END};
const uint16_t PROGMEM l1gui[] = { LT(1,KC_TAB), KC_O, COMBO_END};
const uint16_t PROGMEM l1alt[] = { LT(1,KC_TAB), KC_A, COMBO_END};
const uint16_t PROGMEM l2ctl[] = { LT(4,KC_SPC), KC_H, COMBO_END};
const uint16_t PROGMEM l2sft[] = { LT(4,KC_SPC), KC_T, COMBO_END};
const uint16_t PROGMEM l2gui[] = { LT(4,KC_SPC), KC_N, COMBO_END};
const uint16_t PROGMEM l2alt[] = { LT(4,KC_SPC), KC_S, COMBO_END};
const uint16_t PROGMEM l1cs[] = { LT(1,KC_TAB), KC_E, KC_U, COMBO_END};
const uint16_t PROGMEM l1gs[] = { LT(1,KC_TAB), KC_E, KC_O, COMBO_END};
const uint16_t PROGMEM l2cs[] = { LT(4,KC_SPC), KC_T, KC_H, COMBO_END};
const uint16_t PROGMEM l2gs[] = { LT(4,KC_SPC), KC_T, KC_N, COMBO_END};
const uint16_t PROGMEM l3acutee[] = { LT(3,KC_ESC), KC_E, COMBO_END};
const uint16_t PROGMEM l3gravea[] = { LT(3,KC_ESC), KC_A, COMBO_END};
const uint16_t PROGMEM l3gravee[] = { LT(3,KC_ESC), KC_O, COMBO_END};
const uint16_t PROGMEM l3graveu[] = { LT(3,KC_ESC), KC_U, COMBO_END};
const uint16_t PROGMEM l3circumflexa[] = { LT(3,KC_ESC), KC_QUOT, COMBO_END};
const uint16_t PROGMEM l3circumflexo[] = { LT(3,KC_ESC), KC_COMM, COMBO_END};
const uint16_t PROGMEM l3circumflexe[] = { LT(3,KC_ESC), KC_DOT, COMBO_END};
const uint16_t PROGMEM l3circumflexu[] = { LT(3,KC_ESC), KC_P, COMBO_END};
const uint16_t PROGMEM l3circumflexi[] = { LT(3,KC_ESC), KC_Y, COMBO_END};
const uint16_t PROGMEM l3tremae[] = { LT(3,KC_ESC), KC_J, COMBO_END};
const uint16_t PROGMEM l3tremau[] = { LT(3,KC_ESC), KC_K, COMBO_END};
const uint16_t PROGMEM l3tremai[] = { LT(3,KC_ESC), KC_X, COMBO_END};
const uint16_t PROGMEM l3cedillac[] = { LT(3,KC_ESC), KC_C, COMBO_END};

combo_t key_combos[COMBO_COUNT] = {
  [L1CTL] = COMBO(l1ctl, OSM(MOD_LCTL)),
  [L1SFT] = COMBO(l1sft, OSM(MOD_LSFT)),
  [L1GUI] = COMBO(l1gui, OSM(MOD_LGUI)),
  [L1ALT] = COMBO(l1alt, OSM(MOD_LALT)),
  [L2CTL] = COMBO(l2ctl, OSM(MOD_RCTL)),
  [L2SFT] = COMBO(l2sft, OSM(MOD_RSFT)),
  [L2GUI] = COMBO(l2gui, OSM(MOD_RGUI)),
  [L2ALT] = COMBO(l2alt, OSM(MOD_RALT)),
  [L1CS] = COMBO(l1cs, OSM(MOD_LCTL|MOD_LSFT)),
  [L1GS] = COMBO(l1gs, OSM(MOD_LGUI|MOD_LSFT)),
  [L2CS] = COMBO(l2cs, OSM(MOD_RCTL|MOD_RSFT)),
  [L2GS] = COMBO(l2gs, OSM(MOD_RGUI|MOD_RSFT)),
  [L3ACUTEE] = COMBO(l3acutee, RALT(KC_E)),
  [L3GRAVEA] = COMBO(l3gravea, GRAVEA),
  [L3GRAVEE] = COMBO(l3gravee, GRAVEE),
  [L3GRAVEU] = COMBO(l3graveu, GRAVEU),
  [L3CIRCUMFLEXA] = COMBO(l3circumflexa, CIRCUMFLEXA),
  [L3CIRCUMFLEXO] = COMBO(l3circumflexo, CIRCUMFLEXO),
  [L3CIRCUMFLEXE] = COMBO(l3circumflexe, CIRCUMFLEXE),
  [L3CIRCUMFLEXU] = COMBO(l3circumflexu, CIRCUMFLEXU),
  [L3CIRCUMFLEXI] = COMBO(l3circumflexi, CIRCUMFLEXI),
  [L3TREMAE] = COMBO(l3tremae, TREMAE),
  [L3TREMAU] = COMBO(l3tremau, TREMAU),
  [L3TREMAI] = COMBO(l3tremai, TREMAI),
  [L3CEDILLAC] = COMBO(l3cedillac, RALT(KC_COMM)),
};

/* THIS FILE WAS GENERATED!
 *
 * This file was generated by qmk json2c. You may or may not want to
 * edit it directly.
 */

// const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
// 	[0] = LAYOUT_split_3x5_2(KC_QUOT, KC_COMM, KC_DOT, KC_P, KC_Y, KC_F, KC_G, KC_C, KC_R, KC_L, KC_A, KC_O, KC_E, KC_U, KC_I, KC_D, KC_H, KC_T, KC_N, KC_S, KC_SCLN, KC_Q, KC_J, KC_K, KC_X, KC_B, KC_M, KC_W, KC_V, KC_Z, LT(3,KC_ESC), LT(2,KC_TAB), LT(1,KC_SPC), LT(1,KC_ENT)),
// 	[1] = LAYOUT_split_3x5_2(KC_GRV, KC_EQL, KC_MINS, KC_PLUS, KC_NO, KC_BSLS, KC_SLSH, KC_PERC, KC_LBRC, KC_RBRC, KC_EXLM, KC_QUES, KC_CIRC, KC_DLR, KC_TILD, KC_AT, KC_ASTR, KC_HASH, KC_LPRN, KC_RPRN, KC_NO, KC_NO, KC_UNDS, KC_PIPE, KC_AMPR, KC_NO, KC_NO, KC_NO, KC_LCBR, KC_RCBR, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS),
// 	[2] = LAYOUT_split_3x5_2(KC_HOME, KC_END, KC_PGDN, KC_PGUP, KC_CAPS, KC_NO, KC_MNXT, KC_MPRV, KC_MPLY, KC_NO, KC_LEFT, KC_RGHT, KC_DOWN, KC_UP, KC_DEL, KC_NO, KC_RCTL, KC_RSFT, KC_RGUI, KC_RALT, KC_WH_L, KC_WH_R, KC_WH_D, KC_WH_U, KC_INS, KC_MUTE, KC_VOLU, KC_VOLD, KC_BRIU, KC_BRID, KC_TRNS, KC_TRNS, KC_BSPC, KC_ENT),
// 	[3] = LAYOUT_split_3x5_2(KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_9, KC_5, KC_6, KC_7, KC_8, KC_LALT, KC_LGUI, KC_LSFT, KC_LCTL, KC_NO, KC_0, KC_1, KC_2, KC_3, KC_4, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_PEQL, KC_PPLS, KC_PMNS, KC_PSLS, KC_PAST, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS)
// };

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	[0] = LAYOUT_split_3x5_2(KC_QUOT, KC_COMM, KC_DOT, KC_P, KC_Y, KC_F, KC_G, KC_C, KC_R, KC_L, KC_A, KC_O, KC_E, KC_U, KC_I, KC_D, KC_H, KC_T, KC_N, KC_S, KC_SCLN, KC_Q, KC_J, KC_K, KC_X, KC_B, KC_M, KC_W, KC_V, KC_Z, LT(3,KC_ESC), LT(1,KC_TAB), LT(4,KC_SPC), LT(2,KC_ENT)),
	[1] = LAYOUT_split_3x5_2(KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_BSLS, KC_SLSH, KC_PERC, KC_LBRC, KC_RBRC, KC_LALT, KC_LGUI, KC_LSFT, KC_LCTL, KC_NO, KC_AT, KC_ASTR, KC_HASH, KC_LPRN, KC_RPRN, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_LCBR, KC_RCBR, KC_TRNS, KC_TRNS, KC_SPC, KC_ENT),
	[2] = LAYOUT_split_3x5_2(KC_HOME, KC_END, KC_PGDN, KC_PGUP, KC_INS, KC_NO, KC_MPRV, KC_MPLY, KC_MNXT, KC_NO, KC_LEFT, KC_RGHT, KC_DOWN, KC_UP, KC_CAPS, KC_NO, KC_RCTL, KC_RSFT, KC_RGUI, KC_RALT, KC_WH_L, KC_WH_R, KC_WH_D, KC_WH_U, KC_NO, KC_MUTE, KC_VOLU, KC_VOLD, KC_BRIU, KC_BRID, KC_DEL, KC_BSPC, KC_TRNS, KC_TRNS),
	[3] = LAYOUT_split_3x5_2(KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_9, KC_5, KC_6, KC_7, KC_8, KC_LALT, KC_LGUI, KC_LSFT, KC_LCTL, KC_NO, KC_0, KC_1, KC_2, KC_3, KC_4, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_PEQL, KC_PPLS, KC_PMNS, KC_PSLS, KC_PAST, KC_TRNS, KC_TRNS, KC_SPC, KC_ENT),
	[4] = LAYOUT_split_3x5_2(KC_GRV, KC_EQL, KC_MINS, KC_PLUS, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_EXLM, KC_QUES, KC_CIRC, KC_DLR, KC_TILD, KC_NO, KC_RCTL, KC_RSFT, KC_RGUI, KC_RALT, KC_NO, KC_NO, KC_UNDS, KC_PIPE, KC_AMPR, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_ESC, KC_TAB, KC_TRNS, KC_TRNS)
};

// layer_state_t layer_state_set_user(layer_state_t state) {
//   return update_tri_layer_state(state, 1, 2, 3);
// }
